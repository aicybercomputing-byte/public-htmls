...
const EXCLUDE_RELATIVE_FILES = new Set(["index.html", "preview.html"]);
...
function previewUrl(relativePath) {
  return `preview.html?f=${encodeURIComponent(relativePath)}`;
}

function renderPageItem(page) {
  const href = escapeHtml(previewUrl(page.relative));
  const label = escapeHtml(page.label);
  const path = escapeHtml(page.relative);
  return `
      <li class="page-item">
        <details>
          <summary>
          ...
          <div class="preview-wrap">
            <iframe class="page-preview" data-preview-src="${path}" title="Preview of ${label}" loading="lazy"></iframe>
          </div>
        </details>
      </li>`;
}
...
<body>
  <div class="wrap">
    ...
    ${sections}
    <footer>
      Regenerated by <code>scripts/generate-index.js</code> when repo files change (except <code>index.html</code> and <code>README.md</code>).
      Run <code>node scripts/generate-index.js</code> manually after <code>git fetch</code>.
    </footer>
  </div>
  <script src="preview.js"></script>
  <script>
    AssetPreview.bindPreviewDetails(document);
  </script>
</body>
</html>
`;
